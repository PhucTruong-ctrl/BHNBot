    ...
    return (
        <div className="stats-grid">
            ...
            <StatCard title={stats.cpu.model} icon={Cpu} colorVar="var(--accent-error)">
                ...
                <ProgressBar value={stats.cpu.usage} color="var(--accent-error)" />
            ...
            <StatCard title="RAM" icon={MemoryStick} colorVar="var(--accent-secondary)">
                 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '8px' }}>
                    <div className="stat-value" style={{ color: 'var(--accent-secondary)', fontSize: '1.8rem' }}>
                        {stats.memory.ram.percent.toFixed(1)}%
                    </div>
                    <div className="stat-label" style={{ fontFamily: 'var(--font-mono)' }}>
                         {formatBytes(stats.memory.ram.used)}
                    </div>
                </div>
                <ProgressBar value={stats.memory.ram.percent} color="var(--accent-secondary)" />
                
                <div className="stat-label" style={{ marginTop: '10px' }}>
                    <span>Total: {formatBytes(stats.memory.ram.total)}</span>
                   {stats.memory.swap.total > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginTop: '4px' }}>
                            <span style={{ minWidth: '40px' }}>SWAP</span>
                            <ProgressBar value={stats.memory.swap.percent} color="var(--text-dim)" height="4px" />
                        </div>
                    )}
                </div>
            </StatCard>
            ...
            <StatCard title="Network" icon={Activity} colorVar="var(--accent-success)">
                ...
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: stats.bot.status === 'online' ? 'var(--accent-success)' : 'var(--accent-error)', boxShadow: stats.bot.status === 'online' ? '0 0 8px var(--accent-success)' : 'none' }} />
                     ...
                     <span className="stat-label" style={{ fontFamily: 'var(--font-mono)' }}>
                        {stats.bot.status === 'online' ? formatUptime(stats.bot.uptime) : 'OFFLINE'}
                    </span>
                </div>
            </StatCard>
             ...
             <StatCard title="Storage" icon={HardDrive} colorVar="var(--accent-primary)">
                ...
                <ProgressBar value={stats.disk.percent} color="var(--accent-primary)" />
