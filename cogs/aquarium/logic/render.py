...
class RenderEngine:
    ...
    # Mapping Decor IDs to Emojis (Hardcoded for now - can sync with constants later)
    ICONS = {
    ...
    def generate_view(slots: List[Optional[str]]) -> str:
        ...
        """
        # Ensure we have 5 elements (auto-pad)
        safe_slots = slots[:5] + [None] * (5 - len(slots))
        
        # Convert IDs to Visuals
        visuals = [RenderEngine.ICONS.get(item, '❓') for item in safe_slots]
