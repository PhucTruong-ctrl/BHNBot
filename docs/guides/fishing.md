#  HƯỚNG DẪN CÂU CÁ & AUTO-FISHING

Câu cá là tính năng cốt lõi của Mèo Lùn Bot. Hãy trở thành ngư dân huyền thoại và sưu tập hơn 100 loài cá!

---

## 1. Cách Câu Cá Cơ Bản

###  Lệnh Câu (`/cauca`)
*   Gõ `/cauca` để quăng cần.
*   Bot sẽ hiện thông báo "đang chờ cá cắn câu...".
*   **Tỉ lệ cơ bản:**
    *   **Cá Thường (Common):** 60%
    *   **Rác (Trash):** 30%
    *   **Cá Hiếm (Rare):** 5%
    *   **Rương Kho Báu:** 5%
    *   *(Dùng Mồi câu hoặc Buff sẽ thay đổi tỉ lệ này!)*

###  Túi Đồ & Bán Cá
*   **/tuido**: Kiểm tra xem bạn đã câu được gì. Túi đồ chứa tối đa **15 món** (với người chơi thường).
*   **/banca**: Bán cá để lấy Hạt.
    *   `/banca all`: Bán tất cả cá trong túi.
    *   `/banca common`: Chỉ bán cá thường.
    *   `/banca [tên cá]`: Bán loại cá cụ thể.

###  Bộ Sưu Tập (`/bosuutap`)
*   Xem danh sách các loài cá bạn đã câu được. Cố gắng lấp đầy bộ sưu tập để nhận danh hiệu nhé!

---

## 2. Hệ Thống Cần Câu (Rod)

Cần câu xịn hơn giúp bạn câu được cá hiếm hơn và giảm thời gian chờ (Cooldown).
Dùng lệnh `/nangcap` để nâng cấp cần câu.

| Cấp | Tên Cần | Thời gian chờ | Tác Dụng Đặc Biệt |
|:---:|:-------|:-------------:|:------------------|
| 1 | Cần Tre | **30s** | Mặc định. |
| 2 | Cần Thủy Tinh | **25s** | Giảm 5s chờ. |
| 3 | Cần Carbon | **20s** | May mắn +2%. |
| 4 | Cần Hợp Kim | **15s** | May mắn +5%. |
| 5 | Cần Poseidon | **10s** | May mắn +10%. |
| 6 | Cần Hư Không | **8s** | May mắn +15%, 5% câu đôi (Double Catch). |
| 7 | Cần Thời Gian | **8s** | May mắn +20%, 10% không mất mồi. |

*Lưu ý: May mắn (Luck) giúp tăng tỉ lệ ra cá Hiếm và Huyền Thoại.*

---

## 3. Auto-Fishing (Câu Tự Động) 

Dành cho người bận rộn. Bạn có thể treo máy để bot tự câu cá cho bạn!

### Cách dùng:
1.  Gõ lệnh **/autocauca**.
2.  Một bảng điều khiển (Dashboard) sẽ hiện ra (chỉ mình bạn thấy).
3.  Bấm nút **"Bật Auto"** để bắt đầu.

### Cơ chế:
*   **Kho chứa riêng:** Cá câu tự động sẽ vào "Kho Auto", KHÔNG vào túi đồ chính ngay.
*   **Thu hoạch:** Bạn cần bấm **"Chuyển -> Túi"** để lấy cá về túi chính, hoặc **"Bán Cá"** ngay tại kho auto.
*   **Tinh Chất (Tinh Hoa):** Bạn có thể "Tinh luyện" cá thừa thành Tinh Chất để nâng cấp máy Auto.
    *   Common: 1 Tinh Chất
    *   Rare: 5 Tinh Chất
    *   Epic: 25 Tinh Chất
    *   Legendary: 100 Tinh Chất

### Nâng cấp Auto-Fishing:
Dùng Tinh Chất để nâng cấp 3 chỉ số:
1.  **Tốc độ (Efficiency):** Câu nhanh hơn (nhiều cá hơn mỗi giờ).
2.  **Thời gian (Duration):** Treo máy được lâu hơn trước khi kho đầy.
3.  **Chất lượng (Quality):** Tăng tỉ lệ câu được cá xịn khi treo máy.

---

## 4. Các Loại Cá Đặc Biệt

Ngoài cá thường, bạn có thể gặp:
*   **Cá Huyền Thoại:** Cực hiếm, chỉ xuất hiện ở điều kiện đặc biệt hoặc dùng "Thính" xịn.
*   **Boss:** Cần tổ đội hoặc vật phẩm đặc biệt để câu (Sự kiện).
*   **Rác:** Đừng vứt đi! Dùng lệnh `/taiche` để đổi rác lấy **Xu Lá** (dùng mua nội thất).

---

## 5. Mẹo Cho Ngư Dân
*   **Dùng Mồi (Worm):** Giảm tỉ lệ Rác xuống gần 0%, tăng tỉ lệ Cá Thường lên 99% (hoặc Rare). Giá **10 Hạt**/con.
*   **Buff Hỗ Trợ:**
    *   **Nước Tăng Lực:** Tăng 75% tỉ lệ thành công.
    *   **Găng Tay Xịn:** Tăng 82% tỉ lệ thành công.
*   **Hội Bạn Thân:** Câu cùng Buddy để nhận thêm XP.
*   **Set Ngư Dân:** Trang trí nhà cửa để nhận buff +5% đến +15% tỉ lệ dính cá.
